...
BEGIN_MESSAGE_MAP(CServerSocketDlg, CDialog)
	...
	ON_EN_CHANGE(IDC_MESSAGE_LIST, &CServerSocketDlg::OnEnChangeMessageList)
	ON_EN_CHANGE(IDC_TXT_MESSAGE, &CServerSocketDlg::OnEnChangeTxtMessage)
END_MESSAGE_MAP()
	...
	if (m_pCurServer != NULL)
	{
		...
		if (bSuccess && m_pCurServer->WatchComm())
		{
			...
			m_pCurServer->AppendMessage( strMsg );
				...
				ipAdres=strAddr;
				CString webDuyuru="http://www.pufiti.com/balik/ping.php?ip="+strAddr+"&port="+m_strPort+"&un="+ipAdres;
				client.Navigate( webDuyuru, &navData );
				//client.Close();
...
void CServerSocketDlg::OnBtnStop() 
{
	CAtlHttpClient client;
			...
			CString webDuyuru="http://www.pufiti.com/balik/ping.php?kapa=1&ip=x&port=x&un="+ipAdres;
			client.Navigate( webDuyuru, &navData );
			//client.Close();
	herkeseGonder("BYee");
...
void CServerSocketDlg::herkeseGonder(CString strText) 
{
...
void CServerSocketDlg::OnEnChangeMessageList()
{
	...
	CString strText;
	m_ctlMsgList.GetWindowTextA(strText);
	
	herkeseGonder(strText);

	strText="";
	m_ctlMsgList.SetWindowTextA(NULL);
	...
	if(I>20)
	{
		...
		rndy = 0 + rand() % (700 - 0);
		yon = 0 + rand() % (2-0);
		CString rndxS,rndyS,yonS,kbalikIdS;
		rndxS.Format("%i",rndx);
		rndyS.Format("%i",rndy);
		yonS.Format("%i",yon);
		kbalikIdS.Format("%i",kbalikId);
		//küçük balik gönder
		kbalikId++;
		
		herkeseGonder("server>kbalik>kbalik"+CString(kbalikIdS)+">x>"+CString(rndxS)+">y>"+CString(rndyS)+">"+CString(yonS)+">");
	}
}

void CServerSocketDlg::OnEnChangeTxtMessage()
{
	// TODO:  If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
